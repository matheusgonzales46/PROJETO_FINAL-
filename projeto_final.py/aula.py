...
def inserir_usuario():
   ...
   endereço = entry_endereço.get().strip()
    
   if nome and email and telefone and endereço:
    #    try:
            conn  =  conectar()
            c = conn.cursor()
            c.execute('INSERT INTO usuarios (nome, email, telefone, endereço) VALUES (?,?,?,?)', (nome, email, telefone, endereço))
            conn.commit()
            conn.close()
            ...
            entry_endereço.delete(0, tk.END)
    ...
    if selecao:
        ...
        if  novo_nome and novo_email:
            try:
                    conn  =  conectar()
                    c = conn.cursor()
                    c.execute('UPDATE usuarios SET nome = ?,  email = ? WHERE id = ?', (novo_nome, novo_email, novo_telefone, novo_endereço, user_id))
                    conn.commit()
                    conn.close()
                    messagebox.showinfo('Dados','DADOS INSERIDOS COM SUCESSO!')
                    mostra_usuario()
...
janela.title('...COMERCIOS MATHEUS...')
...
titulo = ttk.Label(main_frame, text='cadastre-se no comercio:', font=('Segoe UI', 10, 'bold'))
...
# textos para direcionar
# NOME
ttk.Label(input_frame, text='EMAIL').grid(row=1, column=0, padx=(0,10), pady=5, sticky='e')
...
entry_endereço = ttk.Entry(input_frame, width=30)
entry_endereço.grid(row=3, column=1, padx=(0,20), pady=5, sticky='w')
